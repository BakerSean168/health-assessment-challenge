...
- `Quality`: ESLint, Next route type generation + TypeScript, 67 unit/component tests, and production `next build`;
- `PostgreSQL integration`: starts the disposable PostgreSQL 17 Compose service from a clean runner, applies all committed migrations, resets aggregate test data, then runs 35 integration tests;
